 ...
 */
class Tx_AdGoogleMapsPluginAddress_PluginAdapter_CoordinatesProvider_Addresses extends Tx_AdGoogleMaps_PluginAdapter_CoordinatesProvider_AbstractCoordinatesProvider {
	...
	public function load() {
		...
		foreach ($this->addresses as $address) {
			if (($coordinate = $address->getTxAdgooglemapspluginaddressCoordinates())) {
				...
				if (($coordinate = $this->layerBuilder->getGoogleMapsPlugin()->getCoordinatesByAddress($addressQuery)) !== NULL) {
					$address->setTxAdgooglemapspluginaddressCoordinates($coordinate);
					$this->data[] = $address->_getCleanProperties();
					$this->coordinates[] = $coordinate;
				}
			}
...
//		$this->addresses = $layer->getTxAdgooglemapspluginaddressAddresses();
		...
		$this->addresses = (count($result) > 0 ? $result[0]->getTxAdgooglemapspluginaddressAddresses() : array());
	}
}
